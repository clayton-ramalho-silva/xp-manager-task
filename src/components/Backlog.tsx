...
export default function Backlog({ project }: BacklogProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !viewingStory?.tasks) return;

    const oldIndex = viewingStory.tasks.findIndex((t) => t.id === active.id);
    const newIndex = viewingStory.tasks.findIndex((t) => t.id === over.id);

    const newTasks = arrayMove(viewingStory.tasks, oldIndex, newIndex) as Task[];
    
    // Optimistic update
    setViewingStory({ ...viewingStory, tasks: newTasks });
